...
| 2026-09-09 | BACKEND | Draft voucher preview | `/vouchers/{id}/image` + `/pdf` returned 404 until first send | Drafts (and sent vouchers whose files are missing on disk) are rendered on the fly via `pdf_module.render_preview()` — temp folder, no QR, nothing saved, status unchanged. Preview buttons added on the voucher, list, and orderer pages | Koby: "allow show voucher even before issued" | remove `_needs_preview` branches in routers/vouchers.py + `render_preview`; revert template buttons |
| 2026-09-09 | NETWORK | New-voucher e-mail notification | none (EMAIL sending only recorded) | `app/mailer.py`: on first send, background thread mails kobysheftel@gmail.com from whereiskoby@gmail.com (Gmail SMTP 587 STARTTLS) with voucher details + PDF attached (JPEG dropped 2026-09-09 on Koby's request). Recipient in config/settings.json `notify_email`; password from env `VOUCHER_SMTP_PASS` → `config/smtp.json` → Windows keyring `smtp_soulwaves`. Tests disable via `VOUCHER_DISABLE_EMAIL=1` (tests/conftest.py) | Koby: "send email ... for new vouchers once produced" | remove mailer call in rules.first_send; delete app/mailer.py + config/smtp.json |
